 ...
 */
 ...
 public function ajax_shiftQuilt() {
  $this->setAjaxMode();

  // Optional: Check if this is allowed or restrict to the active player
  $playerId = self::getCurrentPlayerId();
  
  // Example: shift the player's own personal quilt left
  $direction = self::getArg('shiftDirection', AT_alphanum, false);

  $this->game->shiftQuilt($direction);
  //var_dump($direction);

  $this->ajaxResponse();
}

public function ajax_chooseAssistant()
{
    self::setAjaxMode();
    $option = self::getArg("option", AT_posint, true); // e.g. 1 or 2
    $player_id = self::getCurrentPlayerId();

    $this->game->chooseAssistant($player_id, $option);
